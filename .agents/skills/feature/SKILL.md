...
- Identify which subsystems are involved: core engine (`src/core/`), storage (`src/storage/`), AI/agents (`src/ai/`), server (`src/server/`), testing (`src/testing/`), observability (`src/observability/`), workers (`src/workers/`), browser runtime (`src/service-worker/`).
...
- Prefer Bun built-ins over Node equivalents (see AGENTS.md table).
