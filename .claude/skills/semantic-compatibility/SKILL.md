...
- Adding or deleting reserved workflow metadata markers such as `wf-has-services:` or `schedule-run:`; marker writes, reads, scheduled-occurrence starts, cleanup, purge, and retention must stay aligned.
...
- Changing durable execution-token storage or wire shapes, including `WorkflowState.workflowExecutionToken`, `WorkflowContext.workflowExecutionToken`, activity/finalizer attempt tokens, WebSocket task fields, or long-poll claim/result token echoes.
...
15. For services markers, prove the marker is presence-only, is written atomically with start records and scheduled occurrences, gates resolver calls on recovery, and is deleted by terminal cleanup, purge, and retention. For schedule-run metadata, prove scheduled starts persist the schedule id plus optional occurrence, decode old string records, and sweep the metadata through the same cleanup, purge, and retention paths.
...
21. For workflow execution tokens, preserve per-run semantics: fresh starts mint a token, recovery keeps it stable, terminal replacement and forked runs get a different token, and activity/finalizer attempt tokens stay derived per attempt rather than reused across retries.
22. For persisted workflow version metadata, write only `versionTuple` on fresh state, lift old flat `version` / `agentVersion` / `toolVersions` records through `decodeWorkflowState()`, route diagnostics through the decoder, and regenerate replay/checkpoint fixtures only after verifying the diff is shape-only.
23. For versioning changes, keep `checkVersionCompatibility()` to compatible/incompatible outcomes, keep `weft version:check` to safe/unsafe reporting, and do not reintroduce `migrate`, `migrateCheckpoint`, or `needs-migration` surfaces unless the task explicitly restores them with storage fixtures.
...
25. For unknown persisted workflow-state fields, add neutral extra-field fixtures that prove decode drops the field and resumes with only current `WorkflowState` keys. Do not reintroduce tenant-specific, legacy, migration, or alias normalization when the current contract is tolerate-and-strip. For removed checkpoint fields such as `pendingSignals`, name helpers and tests around `retired` fields instead of old-shape compatibility.
...
- For services marker or schedule-run metadata changes, run the recovered-services, workflow-services, schedule-run-metadata, delayed-start, schedule, purge, and retention tests that prove marker lifecycle across storage paths.
...
- For workflow execution-token changes, run `src/core/engine/execution-tokens.test.ts`, worker context propagation tests, remote-worker protocol parser tests, and any replay/checkpoint fixture review needed to prove the token diff is intentional.
