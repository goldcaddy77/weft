 ...
 * scoped-bulk constraints. All three failure paths — tag coercion, filter
 * build, scoped assert — surface as `invalidParamsFault(faultMessage)` to
 * match the per-file behavior across bulk delete / cancel / signal and the
 * filter portion of bulk mutate-tags.
 *
...
export function validatedListFilterFromBulkInput(input: BulkListFilterInput): ListFilter {
  ...
  try {
    filter = listFilterFromBulkInput(tags === undefined ? input : { ...input, tags });
  } catch (error) {
    throw invalidParamsFault(faultMessage(error));
  }
