...
/** Capability profile as serializable JSON from in-page evaluate. */
type SerializedCapabilities = {
  readonly readAfterWrite: string;
  readonly scanConsistency: string;
...
describe('IndexedDBStorage — real Chromium durability', () => {
  beforeAll(async () => {
    if (!shouldRun) return;
  ...
  (shouldRun ? it : it.skip)(
    'capabilities() in a real browser matches the declared contract',
    async () => {
      ...
      try {
        const result = (await page.evaluate((databaseName) => {
          ...
          return caps;
        }, uniqueDatabase())) as SerializedCapabilities;
  ...
  (shouldRun ? it : it.skip)(
    'put/get/keys/count/deletePrefix round-trip real bytes through the structured-clone boundary',
    async () => {
      const page = await openPage();
  ...
  (shouldRun ? it : it.skip)(
    'records written on first page load survive page.reload() and are readable by a fresh adapter',
    async () => {
      const page = await openPage();
      const dbName = uniqueDatabase();
      try {
  ...
  (shouldRun ? it : it.skip)(
    'scan() yields keys in ascending lexicographic order in a real IDB',
    async () => {
      const page = await openPage();
      const dbName = uniqueDatabase();
      try {
  ...
  (shouldRun ? it : it.skip)(
    'keys() with reverse:true yields keys in descending order in a real IDB',
    async () => {
      const page = await openPage();
      const dbName = uniqueDatabase();
      try {
  ...
  (shouldRun ? it : it.skip)(
    'batch() applies all operations atomically — all visible together, none missing',
    async () => {
      const page = await openPage();
      const dbName = uniqueDatabase();
      try {
  ...
  (shouldRun ? it : it.skip)(
    'conditionalBatch() commits when conditions match and aborts when they do not',
    async () => {
      const page = await openPage();
      const dbName = uniqueDatabase();
      try {
