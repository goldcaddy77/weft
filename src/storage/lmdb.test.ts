...
/** Create a temporary directory for each test's LMDB data. */
function createTempPath(): string {
  return mkdtempSync(join(tmpdir(), 'weft-lmdb-test-'));
}

describe('LMDBStorage', () => {
  const cleanupPaths: string[] = [];

  function createStorage(path?: string): LMDBStorage {
    const p = path ?? createTempPath();
    cleanupPaths.push(p);
    return new LMDBStorage(p);
  }
  ...
  it('[Symbol.dispose] closes the LMDB environment', () => {
    const storage = createStorage();
    storage[Symbol.dispose]();
    // After dispose, the environment is closed. Operations should throw.
