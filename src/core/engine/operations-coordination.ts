...
export async function processWaitSignalOperation(
  ...
  while (true) {
    ...
    const existingPayload = await prepareSignalConsumeForCheckpointCommit(
      internals,
      workflowId,
      operation.signalName,
    );
    if (existingPayload.found) {
      appendCheckpointCommitSideEffects(internals, workflowId, {
        operations: existingPayload.operations,
      });
    ...
    const bufferedPayload = await prepareSignalConsumeForCheckpointCommit(
      internals,
      workflowId,
      operation.signalName,
    );
    if (bufferedPayload.found) {
      ...
      appendCheckpointCommitSideEffects(internals, workflowId, {
        operations: bufferedPayload.operations,
      });
