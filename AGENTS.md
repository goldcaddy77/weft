...
- `ctx.sleep()` timer identity must stay replay-stable: the durable sleep operation id is `${workflowId}:${step}` so crash recovery re-arms the same timer key. Because `start-new` can reuse the workflow id and step, sleep settlement must remain deadline-aware (`fireAt`) and must not use a missing `timer-idx:sleep:` index as proof that the current run's timer fired; stale earlier-run timers can delete that shared index. Fired sleep timers are acknowledged only after the awakened inline workflow commits a later durable operation or reaches terminal state; callback failure, suspension, termination, checkpoint failure, and engine disposal must settle pending acknowledgement waiters without deleting the only retryable timer record prematurely.
...
- `decodeWorkflowState()` strips unknown persisted fields instead of preserving retired or foreign keys. Fresh state writes must stay on the current `WorkflowState` allowlist; cleanup fixtures should prove tolerated extras are dropped without reintroducing alias, tenant, legacy, or migration wording. Checkpoint cleanup helpers should use `retired` naming for tolerated removed fields such as `pendingSignals`.
