...
The schema fields are introspection metadata. Core workflow execution does not validate workflow input or output from these fields unless an adapter explicitly opts into that validation. Agent definitions can also be registered directly with optional agent registration options.
