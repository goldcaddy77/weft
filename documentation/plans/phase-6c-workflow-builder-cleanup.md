...
5. **Package publication status check**. Record in the PR description: `package.json` `name`, `version`, `private` flag, and `publishConfig`. weft today is `name: "weft"`, `version: "0.1.0"`, `publishConfig.access: "public"`, no `private` flag. This is a published-on-publish-cadence pre-1.0 package; semver allows breaking changes in the 0.x line. A CHANGELOG entry ships with this PR if `CHANGELOG.md` exists in the repo; otherwise the PR description must include a dedicated Migration section. The entry MUST enumerate the removed overloads (`engine.register(name, handler)`, `engine.register(name, registration)`, `engine.register(activityDefinition)`, `workflow(handler)`) and the removed types (`WorkflowRegistration`, `WorkflowDefinitionOptions`, the global `ActivityTypes` augmentation interface, `UnknownActivityNameWhenRegistryIsEmpty`) so consumers get an unambiguous upgrade path.
