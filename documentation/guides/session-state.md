...
`run(fn, ...args)` executes a function as a durable operation that is automatically routed through sticky worker execution. This is useful for work that benefits from co-location with session-bound context.
