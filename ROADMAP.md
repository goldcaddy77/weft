...
- [ ] **Close the activity-completion throughput gap to the architecture target.**
  ...
  **Where:** terminal workflow cleanup in `src/core/engine/termination.ts` and hot-path storage writes around activity completion.

  The architecture target is `>30K/sec` activity completions on a single node with SQLite. The remaining suspected gap is terminal scratch cleanup on the completion hot path plus SQLite synchronization cost; prefer batching, coalescing, or deferring cleanup over weakening the target.
  ...
  **Acceptance criteria:**
  ...
  - Verification passes with `bun run lint`, `bun run typecheck`, targeted tests for terminal cleanup and storage-write changes, and `bun run verify:documentation`.
