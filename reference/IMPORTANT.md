...
- [~] **Performance targets measured against spec** (2026-04-07, updated 2026-04-30): Re-measured after optimizations. Findings:
  ...
  - **Sustained-load RSS stability**: spec `stable RSS under sustained 10K workflows/sec`, measured on 2026-05-02 by `src/benchmarks/load-growth-memory.test.ts` at `~16.0K/sec` with a post-warmup RSS slope of `~119KB/sec` against a `256KB/sec` threshold → **meets spec**. The harness uses SQLite plus short retention windows so the benchmark measures steady-state engine churn instead of intentionally retained terminal history.
