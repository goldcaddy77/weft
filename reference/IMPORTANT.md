...
- [x] **`MetricsCollector` histogram arrays grow without bound** (2026-04-10): Replaced unbounded `number[]` with a fixed-capacity `CircularBuffer` backed by `Float64Array`. Memory is now O(capacity) per histogram, not O(observations). Percentiles computed from a sorted copy of only the live portion.

- [x] **`PromptCache.#evictOldest` creates quadratic intermediate arrays** (2026-04-10): Replaced single-pass DFS that spread `[...ancestors, node]` per stack entry with a two-pass approach: Pass 1 finds the oldest terminal with zero ancestor allocations; Pass 2 (`#findPath`) walks from root to the target to build the ancestor chain at O(depth) cost.

- [x] **`PromptCache` uses FIFO eviction instead of LRU** (2026-04-10): Re-inserting an existing terminal now refreshes `node.sequence` on the early-return path, giving LRU semantics so frequently-used prefixes are evicted last.

- [x] **`validateRegistrations` mislabels standalone activities** (2026-04-10): Already fixed in a prior commit; added regression test asserting `workflowType === '(standalone)'` on all issues from explicitly-passed activities.

- [x] **Shared `BudgetTracker` race in `supervise` finally block** (2026-04-10): The finally block now checks `budget.signal === controller.signal` before detaching — only the call that owns the current controller replaces it, so concurrent `supervise` calls no longer clobber each other.
