...
- [~] **Tenant context in worker-execution mode.** `WorkerExecutionStrategy` still drops the resolved tenant across the `postMessage` boundary. Stop-gap landed 2026-04-07: the `Engine` constructor throws when both `workerExecution` and `tenantResolver` are configured, closing the isolation bypass. Full fix still requires extending `WorkerInboundMessage.run` to carry the tenant and constructing an engine-equivalent `Context` inside the worker-side runner.
