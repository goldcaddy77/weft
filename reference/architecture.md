...
- [x] **Works identically on SQLite, LMDB, and IndexedDB.** Same test suite passes on all three backends. (`src/core/search-attributes-multibackend.test.ts` and `src/core/search-attributes-integration.test.ts` parametrize across all 5 `storageBackends` — Memory, BunSQLite, LMDB, Turso, IndexedDB.)
...
- [x] **Works identically on SQLite, LMDB, and IndexedDB.** Same test suite passes on all three backends. (`src/core/updates-multibackend.test.ts` A7 suite: parametrizes inline `onUpdate`, `waitForUpdate`, timeout, FIFO, and post-cancel rejection across all 5 `storageBackends`.)
...
- [x] **Remote worker example in documentation.** Shows `interceptors: [activity]` on remote worker constructor. (See `docs/guides/remote-workers.md` lines 124–138: `const { activity } = createObservabilityInterceptors()` → `new RemoteWorker({ … interceptors: [activity] })`.)
...
**Track 1 — Foundations (blocking for everything else).** Effect logs at the tool-call boundary (§2 ACRFence). Storage-backed event log with hash-chained writes (§4 ESAA). Compensation handlers on activities + `ctx.saga()` (§3 SagaLLM/Atomix). These three interlock; do them in one sprint or you'll do them twice.

**Track 2 — Testing and diagnosis (can start after Track 1 effect logs land).** Chaos primitives in `TestEngine` (§6 ReliabilityBench). Failure categorization enum (§6 AgentDebug). `weft validate` design-time linter (§6 Cemri). Constraint primitive with `onViolation` semantics (§7 AgentRx).
...
- [ ] `src/core/activity.ts` supports `{ run, compensate, resourceScope, idempotencyKey }` activity definitions; `compensate` is optional but, if present, is registered.
- [ ] `src/core/context.ts` exposes `ctx.saga(steps)` that runs activities in order and, on failure, runs `compensate` in reverse for every successfully-completed step.
- [ ] `bun test src/core/__tests__/saga.test.ts` passes a 3-step saga test where step 3 fails and compensators for step 1 and step 2 run exactly once each, verified across an engine restart.
...
- [ ] `bun test src/core/__tests__/constraints.test.ts` passes a test that a violated constraint with `onViolation: 'compensate'` triggers the workflow's saga compensators.
