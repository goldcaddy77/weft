...
- [ ] **Tenant context in worker-execution mode.** `WorkerExecutionStrategy` currently drops the resolved tenant on `postMessage` (`src/core/worker-execution-strategy.ts:78-94`), so when an agent registered with `validateInput` or `toolsForTenant` runs in worker mode, the hooks observe `ctx.tenant === undefined`. This is a tenant isolation bypass under documented configuration. Closing the gap requires extending `WorkerInboundMessage.run` to carry the tenant and giving the worker-side runner access to engine-equivalent `Context` construction. Stop-gap until then: refuse `Engine` construction when `workerExecution` and `tenantResolver` are both set.
- [x] **Routing policies.** `RoutingPolicy = 'least-loaded' | 'round-robin' | 'fair-share'` in `src/worker/registry.ts`. `WorkerRegistry` constructor accepts `{ policy }`; `findWorker(activity, { fairShareKey })` consults per-worker per-key counts. `least-loaded` and `round-robin` are plumbed through `serve({ routingPolicy })` in `src/server/index.ts`. **`fair-share` caveat**: `TaskDispatch` does not yet expose a `fairShareKey` field, so `serve({ routingPolicy: 'fair-share' })` silently falls back to least-loaded until the dispatch-time hook is added (tracked in `reference/IMPORTANT.md`).
...
- [ ] **AI dashboard detail view (enhancements).** Three additional fragments are still proposed — `agent-cost-waterfall.svelte` (bar chart of turn cost), `agent-conversation.svelte` (rendered message history), and `agent-reasoning-trace.svelte` (accordion over `thinking` blocks). These require additional event plumbing (`AgentTurnCompletedEvent.reasoningTrace` fields) and per-turn message capture beyond what `AgentTurn` currently renders. Deferred until that event data is available end-to-end.
