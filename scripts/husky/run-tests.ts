 ...
 * {@link LOAD_SENSITIVE_TEST_PATHS} entries. Shared by the hook and its
 ...
 */
export function buildTestCommand(testFiles: string[], junitPath: string): string[] {
  return [
    'test',
    '--timeout',
...
export async function runTestSuite(
  ...
  try {
    ...
    const full = await dependencies.runCommand(buildTestCommand(testFiles, fullReportPath));
    ...
    const isolation = await dependencies.runCommand(
      buildTestCommand(failingFiles, isolationReportPath),
    );
